...
class ConsumeBili : AnimeParser() {
    ...
    override suspend fun loadVideoServers(episodeLink: String, extra: Map<String,String>?): List<VideoServer> {
        ...
        return listOf(
            VideoServer(
                name = "Server",
                embed = FileUrl(url = ""),
                extraData = mapOf("sources" to modifiedSources,"subs" to modifiedSubtitles)
            )
        )
    }
    ...
    class BilibiliExtractor(override val server: VideoServer) : VideoExtractor() {
        override suspend fun extract(): VideoContainer {
            val data = server.extraData!!
            val videos = data["sources"]!!.split("\\").map {
                Video(null,VideoType.DASH,it)
            }
            val subs = data["subs"]!!.split("\\").map {
                val s = it.split("::")
                Subtitle(s[0],s[1])
            }
