...
import parsers.anime.NineAnime.Companion.cipher
import java.util.*
...
class VizCloud(override val server: VideoServer) : VideoExtractor() {
    ...
    private val regex = Regex("(.+?/)e(?:mbed)?/([a-zA-Z\\d]+)")
    ...
    private suspend fun encrypt(text: String, steps: String): String {
            ...
            cipherKey = if (cipherKey != null && (lastChecked - System.currentTimeMillis()) < 1000 * 60 * 30) cipherKey!!
                ...
                Mapper.parse<CipherKey>(
                    Base64.getDecoder().decode(
                        Base64.getDecoder().decode(
                            Base64.getDecoder().decode(client.get(jsonLink).text),
